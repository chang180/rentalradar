...
export function InteractiveHeatmap({
    ...
    const heatmapData = useMemo(() => {
        ...
        return data.map((item, index) => ({
            ...
            y:
                ((item.listings - minListings) / (maxListings - minListings)) *
                100,
                    ...
                    <div className="mt-2 space-y-1 text-sm">
                        ...
                        {data.price_per_ping && (
                            <div className="flex justify-between gap-4">
                                ...
                                <span className="font-medium">
                                    ${data.price_per_ping.toLocaleString()}
                                </span>
                            </div>
                        )}
                    </div>
                        ...
                        >
                            {heatmapData.map((entry, index) => (
                                <Cell
                                    ...
                                    stroke={
                                        selectedDistrict === entry.district
                                            ? '#3b82f6'
                                            : 'transparent'
                                    }
                                    strokeWidth={
            ...
            {hoveredPoint && (
                <div className="rounded-lg border border-blue-200 bg-blue-50 p-3 dark:border-blue-500/40 dark:bg-blue-500/10">
                    <p className="text-sm text-blue-800 dark:text-blue-200">
                        <strong>{hoveredPoint.district}</strong> - 熱點分數:{' '}
                        {hoveredPoint.heatScore.toFixed(2)}
                    </p>
                </div>
            )}
        </div>
