import type { MarketForecast, MarketTrendPoint } from '@/types/analysis';
    ...
    const enhancedData = useMemo(() => {
        ...
        return data.map((point, index) => {
            ...
            const change = previous
                ? ((point.average_rent - previous.average_rent) /
                      previous.average_rent) *
                  100
                : 0;
            ...
            const avg =
                recentPeriods.reduce((sum, p) => sum + p.average_rent, 0) /
                recentPeriods.length;
            const variance =
                recentPeriods.reduce(
                    (sum, p) => sum + Math.pow(p.average_rent - avg, 2),
                    0,
                ) / recentPeriods.length;
            ...
            return {
                ...point,
                trend_direction:
                    change > 2 ? 'up' : change < -2 ? 'down' : 'neutral',
                    ...
                    <div className="mt-2 space-y-1 text-sm">
                        ...
                        {data.momentum !== undefined && (
                            <div className="flex justify-between gap-4">
                                ...
                                <span
                                    className={`font-medium ${data.momentum > 0 ? 'text-green-600' : 'text-red-600'}`}
                                >
                                    {data.momentum > 0 ? '+' : ''}
                                    {data.momentum}%
                                </span>
                ...
                <div className="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
                    ...
                    <p
                        className={`mt-1 text-2xl font-bold ${
                            (enhancedData[enhancedData.length - 1]?.momentum ||
                                0) > 0
