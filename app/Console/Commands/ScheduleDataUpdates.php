...
class ScheduleDataUpdates extends Command
{
     ...
     * 執行資料下載
     */
    private array $lastDownloadResult = [];
    ...
    private function executeDataDownload(): array
    {
        ...
        try {
            // 官方僅提供 CSV／XML 兩種批次 ZIP，內容一致，這裡固定下載 CSV 版本
            $csvResult = $this->downloadService->downloadRentalData('csv');
            $this->lastDownloadResult = $csvResult;

            $result = [
                'successful' => $csvResult['success'] ? 1 : 0,
                'failed' => $csvResult['success'] ? 0 : 1,
                'total' => 1,
                'details' => [
                    'csv' => $csvResult,
                ],
    ...
    /**
     * 執行資料解析（內容一律為 ZIP，統一走 parseZipData）
     */
    private function executeDataParsing(): array
    {
        ...
        try {
            if (empty($this->lastDownloadResult['success'])) {
                $this->warn('沒有可解析的下載結果，略過解析。');
            ...
            $parseResult = $this->parserService->parseZipData($this->lastDownloadResult['file_path']);

            $result = [
                'files_processed' => $parseResult['csv_files_count'] ?? 0,
                'successful_records' => $parseResult['processed_count'] ?? 0,
                'failed_records' => $parseResult['error_count'] ?? 0,
                'total_records' => ($parseResult['processed_count'] ?? 0) + ($parseResult['error_count'] ?? 0),
                'data' => $parseResult['data'] ?? [],
            ];

            if (! empty($result['data'])) {
                $this->parserService->saveToDatabase($result['data']);
            }
